...
public class SortByPriceIncreaseCommand implements Command {
    ...
    private static Logger log = LogManager.getLogger(SortByPriceIncreaseCommand.class);
    @Override
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) {
        log.debug("Loading list of dishes sorted by increase price started.");
        ServiceFactory serviceFactory = ServiceFactory.getInstance();
        List<Dish> dishes;
        HttpSession session = request.getSession();
        try{
            dishes = serviceFactory.getDishService().sortByPriceIncrease();
        ...
        log.debug("Loading list of dishes sorted by increase price finished.");
