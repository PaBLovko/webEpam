 ...
 */
public class CarActionService {
    private static Logger logger = LogManager.getLogger(CarActionService.class);
     ...
     */
    public void run(@NonNull Car car) throws ServiceException{
        logger.debug(String.format(METHOD_IS_INVOKED, car));
        if (!car.isFuel() || car.getWheels().size()<4){
            throw new ServiceException("can't go");
        }
    ...
    private void breakWheel(@NonNull Car car){
        logger.debug(String.format(METHOD_IS_INVOKED, car));
        car.getWheels().remove(new Random().nextInt(car.getWheels().size()));
        logger.info(String.format(METHOD_WORKED_CORRECTLY_CAR, car));
    }
